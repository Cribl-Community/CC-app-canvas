...
// Built-in fallback scaffolds in case external files can't be loaded
...
const FALLBACK_INDEX_HTML = (title: string) => `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
const FALLBACK_README = (name: string) => `# ${name}
...
// Load scaffold files from the scaffolds directory, with fallback to built-in defaults
async function loadScaffoldFile(filename: string): Promise<string> {
  try {
    const path = `${import.meta.env.BASE_URL}scaffolds/latest/source/${filename}`;
    ...
    console.warn(`Failed to load scaffold file ${filename}, using fallback:`, err);
    ...
    const error = new Error(`No fallback for scaffold file: ${filename}`);
    error.cause = err;
    throw error;
  }
}
...
export async function buildSourceTgz(meta: ProjectMeta, files: ProjectFiles): Promise<Blob> {
  ...
  // ── package.json ──────────────────────────────────────────────────────────
  // Load from scaffolds directory; update package name to match the app
  const packageJsonText = await loadScaffoldFile('package.json');
  const packageJson = JSON.parse(packageJsonText);
  packageJson.name = appName;
  tar.addFile('package.json', JSON.stringify(packageJson, null, 2));

  // ── tsconfig.json ─────────────────────────────────────────────────────────
  // Load from scaffolds directory
  const tsconfigText = await loadScaffoldFile('tsconfig.json');
  tar.addFile('tsconfig.json', tsconfigText);

  // ── vite.config.ts ────────────────────────────────────────────────────────
  // Load from scaffolds directory
  const viteConfigText = await loadScaffoldFile('vite.config.ts');
  tar.addFile('vite.config.ts', viteConfigText);

  // ── index.html ────────────────────────────────────────────────────────────
  // Use metadata name for title
  tar.addFile('index.html', FALLBACK_INDEX_HTML(meta.name));

  // ── README.md ─────────────────────────────────────────────────────────────
  tar.addFile('README.md', FALLBACK_README(meta.name));
